/*this version implements  the estimation of the  correlation.
It is crappy slow and the correlation is crappily estimated.
*/
  ...
  data {
    ...
    //point estimator of the correlation: 1/(number of folds)
    real rhoHat; 
    
    //flag which enables the estimation of the correlation
    int correlEstimation;
     }
 ...
 transformed parameters {
    ...
    //The determinant of M is analytically known, given rho
    detM <- (1+(Nsamples-1)*rho)*(1-rho)^(Nsamples-1);

    //build invM. It does not depend on the data but it depends on rho.
    for (j in 1:Nsamples){
      for (i in 1:Nsamples){
        if (j==i)
          invM[j,i]<- (1 + (Nsamples-2)*rho)*pow((1-rho),Nsamples-2);
        else
          invM[j,i]<- -rhoHat * pow((1-rho),Nsamples-2);
       }
    }
  ...
  model {
    ...
    if (correlEstimation==1){
       rho ~ uniform (0.01,0.99);
    } else {
      rho ~ normal (rhoHat,0.001);
    }
