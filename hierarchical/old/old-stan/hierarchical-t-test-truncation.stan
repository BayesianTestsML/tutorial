...
Truncation on the sampling of delta0 and of the delta-i
*/
  ...
  parameters {
    ...
    vector<lower=deltaLow,upper=deltaHi>[q] delta;               
 ...
 transformed parameters {
  ...
  model {
    ...
    //we need to de-vectorialize in order to allow for truncation
    for (i in 1:q)
     delta[i]  ~ student_t(nu, delta0, std0) T[deltaLow,deltaHi];
