BayesianSignedRank <- function(diffVector,rope_min,rope_max) {
  ...
  for (rep in 1:samples){
   ...
   winRight[rep] <- max(epsilon, (winRight[rep]==maxWins)*1/winners )
   winRope[rep] <- max (epsilon, (winRope[rep]==maxWins)*1/winners )
   winLeft[rep] <- max(epsilon, (winLeft[rep]==maxWins)*1/winners )
  }
  
  
  results = list ("probLeft"=mean(winLeft), "probRope"=mean(winRope),
                  "probRight"=mean(winRight) )
